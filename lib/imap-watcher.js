...
function stripQuotedReply(text) {
  if (!text) return '';
  // Knip bij Gmail-style quote-attribution (NL/EN)
  const patterns = [
    /\n\s*(?:Op|On)\s+[^\n]+(?:wrote|schreef|skrev|escreveu):\s*\n/i,
    /\n\s*-{2,}\s*(?:Original message|Oorspronkelijk bericht|Original Message)\s*-{2,}/i,
    /\n\s*From:\s+[^\n]+\nSent:\s+[^\n]+\nTo:\s+/i,
    /\n\s*Van:\s+[^\n]+\nVerzonden:\s+[^\n]+\nAan:\s+/i,
    /\n\s*Verzonden vanaf mijn iPhone/i,
    /\n\s*Sent from my iPhone/i,
  ];
  let cut = text.length;
  ...
  let body = text.substring(0, cut);
  // Verwijder lines die met '>' beginnen (quoted reply text)
...
async function handleReply(parsed) {
  ...
  // Detecteer automated mails (out-of-office, mailer-daemon, etc.)
  // — we LOGGEN ze nog wel maar genereren GEEN AI-reply om loops te voorkomen.
  const isAutomated = /auto[- ]?reply|out of office|delivery (failure|status)|undeliverable|mailer-daemon|automatic reply|automatisch antwoord|afwezigheid/i.test(subject)
    || (parsed.headers.get('auto-submitted') && parsed.headers.get('auto-submitted') !== 'no');
  ...
  // Strip quoted reply uit body voor schonere context
  const rawText = parsed.text || (parsed.html ? parsed.html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : '');
  const bodyText = stripQuotedReply(rawText);

  // Log inbound communication ALTIJD (ook auto-replies — Robert wil ze zien)
  db.logCommunication({
    lead_id: lead.id,
    type: 'email',
    direction: 'inbound',
    subject: isAutomated ? `[Auto-reply] ${subject}` : subject,
    body: bodyText,
  ...
  try { require('./lead-sync').syncLeadAsync(lead.id); } catch {}

  // Voor automated replies: stoppen hier — geen stage-shift, geen AI-reply
  if (isAutomated) {
    console.log(`  ↪ Auto-reply van ${fromAddr} gelogd; geen AI-reply, geen stage-shift`);
    return;
  }

  // Echte reply: funnel auto-progress naar engaged
  db.advanceLeadStage(lead.id, 'engaged');
  ...
  // Schrijf naar pending_actions als VOORSTEL (geen auto-send) — wacht op Robert's approval
  db.addPendingAction({
    lead_id: lead.id,
    type: 'email_reply',
    ...
    intent: replyResult.intent || null,
  });

  console.log(`  ↪ AI-reply voorstel klaar voor "${lead.name}" (${fromAddr}) — wacht op approval`);
}
