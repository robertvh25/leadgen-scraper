...
function processCampaigns() {
  ...
  try {
    ...
    for (const camp of active) {
      ...
      // De EERSTE stap (idx 0) van een sequence is altijd auto-send
      // (= cold-outreach eerste mail). Robert hoeft die niet handmatig
      // te approven. Stap 2+ respecteert require_approval.
      const isFirstStep = nextStepIndex === 0;
      const autoSend = isFirstStep ? 1 : (step.require_approval ? 0 : 1);
