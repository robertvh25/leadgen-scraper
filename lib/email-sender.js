...
async function sendEmail({ to, subject, body, leadId, inReplyTo }) {
  ...
  try {
    ...
    // IMAP append naar "Verzonden berichten" is alleen nodig bij Google
    // Workspace SMTP RELAY (smtp-relay.gmail.com) — die slaat niet zelf op.
    // Normale Gmail/Workspace SMTP (smtp.gmail.com) bewaart mails al automatisch
    // in Sent. Zet IMAP_APPEND_SENT=1 als je relay gebruikt; default uit
    // om dubbele mails in je Verzonden te voorkomen.
    if (process.env.IMAP_APPEND_SENT === '1') {
