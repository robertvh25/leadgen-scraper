...
async function generateReply({ lead, inboundSubject, inboundBody }) {
  ...
  // Pak de meest recente outbound mail naar deze lead als context
  const comms = db.getLeadCommunications(lead.id);
  const lastOutbound = comms.find(c => c.direction === 'outbound' && c.type === 'email');
  ...
  if (lastOutbound) {
    userPromptParts.push(`# Mijn vorige uitgaande mail\nOnderwerp: ${lastOutbound.subject || '(geen onderwerp)'}\n\n${lastOutbound.body || ''}`);
  }
  userPromptParts.push(`# Hun reply (zojuist binnen)\nOnderwerp: ${inboundSubject || '(geen)'}\n\n${inboundBody || '(lege body)'}`);
  userPromptParts.push(`# Jouw antwoord (alleen de mail-body):`);
