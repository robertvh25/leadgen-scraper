...
Bied aan: "Ik stuur u zo een persoonlijke briefing-link waarmee u uw pakket kunt kiezen en de gegevens kunt invullen. Mag ik daarvoor kort bevestigen wat u zoekt: nieuwe website, huidige opfrissen, of iets specifieks?"
NOOIT direct een prijs of bedrag in tekst noemen, en NOOIT zelf een URL in de body zetten — Robert genereert de briefing-link handmatig en mailt die daarna apart.
...
INTENT-CODES:
- "direct_start" → lead is overtuigd, wil prijs / starten / briefing. Body bevestigt scope-vraag en zegt dat de briefing-link "zo apart toegestuurd wordt". Zet GEEN URL of placeholder in de body.
- "meeting" → lead is geïnteresseerd maar twijfelend. Body biedt 10-min call aan.
...
async function generateReply({ lead, inboundSubject, inboundBody }) {
  ...
  // Mocht AI per ongeluk een {{briefing_link}} placeholder achterlaten, vervang met holding-tekst.
  const cleanBody = body.replace(/\{\{briefing_link\}\}/g, '(ik stuur u zo de persoonlijke briefing-link)');
  return { intent: parsed.intent || 'other', body: cleanBody };
}
